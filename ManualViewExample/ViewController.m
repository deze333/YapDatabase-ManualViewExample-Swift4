...
#import "Person.h"
#import "ManualViewExample-Swift.h"
...
- (UITableViewCell *)tableView:(UITableView *)sender cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	[roDatabaseConnection readWithBlock:^(YapDatabaseReadTransaction *transaction) {
        ...
        person = [[transaction ext:ManualView_RegisteredName] objectAtIndexPath:indexPath withMappings:self->mappings];
	}];
...
- (void)tableView:(UITableView *)sender didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // NOTE: Experiment via Swift
    
    [SwiftAccess findPersonWithConnection:roDatabaseConnection extName:ManualView_RegisteredName];
    
    // NOTE: Same Objective-C call
    
    /*
    __block Person *person = nil;
    [roDatabaseConnection asyncReadWithBlock:^(YapDatabaseReadTransaction * _Nonnull transaction) {
        ...
        [[transaction ext:ManualView_RegisteredName] enumerateRowsInGroup:group
                                                                       withOptions:options
                                                                        usingBlock:
         ^(NSString *collection, NSString *key, id object, id metadata, NSUInteger index, BOOL *stop){
             
             NSLog(@"Enumerating key: %@, object: %@", key, [object name]);
             if ([[object name] isEqualToString:[person name]]) {
                 NSLog(@"--- Found, stop: %@", [object name]);
                 *stop = YES;
             }
         }];
    }];
